...
export default (env: Environment) => {
  ...
  let bindings = [...Object.entries(pattern), ...Object.entries(core)];

  for (let [name, binding] of bindings) {
    // Allow string shorthands for simple type aliases
    if (typeof binding === "string") {
      binding = { type: binding, value: strudelCore[name] };
    }
...
type Bindings = { [name: string]: string | Omit<BindingSpec, "name"> };

function fromPatternMethod(name: string) {
  return (...args) => {
    if (args.length === 0) {
      throw new Error(`Unexpected invocation of ${name} with 0 arguments.`);
    }

    return args[args.length - 1][name](...args.slice(0, -1));
  };
}

const pattern: Bindings = {
  // TODO:
  // query: "Pattern -> Query -> [Hap]",

  // Pattern Object Functions
  withSteps: {
    type: "(Number -> Number) -> Pattern a -> Pattern a",
    value: fromPatternMethod("withSteps"),
  },

  withValue: {
    type: "(a -> b) -> Pattern a -> Pattern b",
    value: fromPatternMethod("withValue"),
  },
  ...
  slow: "Pattern Number -> Pattern a -> Pattern a",
  late: "Pattern Number -> Pattern a -> Pattern a",
  "~>": {
    type: "Pattern Number -> Pattern a -> Pattern a",
    value: strudelCore.late,
  },
  early: "Pattern Number -> Pattern a -> Pattern a",
  "<~": {
    type: "Pattern Number -> Pattern a -> Pattern a",
  ...
  inside:
    "Pattern Number -> (Pattern a -> Pattern b) -> Pattern a -> Pattern b",
  outside:
    "Pattern Number -> (Pattern a -> Pattern b) -> Pattern a -> Pattern b",
};
